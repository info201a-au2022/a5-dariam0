...
noncountries <- c(
  "Africa", "Africa (GCP)", "Antarctica", "Asia",
...
server <- function(input, output) {
  output$intro <- renderUI({
    p(
      "Climate change is a real issue. With climate change comes effects that
      ...
      data from", em("Our World in Data on CO2"), "and greenhouse gas emissions
      to examine what countries seem to be emitting the most and what countries
  ...
  linechart_plot <- reactive({
    ...
    linechart_plot <- ggplot(data = linechart_data) +
      ...
      labs(
        title = paste0(
          input$y_var, " for ", input$country_continents1,
